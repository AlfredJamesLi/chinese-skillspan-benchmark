#!/usr/bin/env bash
# JobBERT-zh listed-mix 3M: MLM from listed corpus + CRF. Init from 1M listed encoder if present.
set -euo pipefail
...
OUT="${OUT_DIR:-$PAPER/output/jobbert_zh_listed_3m}"
CORPUS="${CORPUS:-$PAPER/data/jobbert_listed_mix_3m_sents.jsonl}"
...
echo "listed-3M start GPU=${CUDA_VISIBLE_DEVICES:-?} nproc=$NGPU corpus=$CORPUS $(date -Is)" | tee -a "$OUT/run.log"
...
echo "listed-3M done $(date -Is) exit=$?" | tee -a "$OUT/run.log"
