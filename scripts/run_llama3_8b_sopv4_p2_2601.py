#!/usr/bin/env python3
"""SOP-v4 extract baseline: local Llama-3-8B-Instruct on all 2601 P2 IDs.

Same prompt as Qwen2.5-14B-Instruct zero-shot. No LoRA.
Does not overwrite frozen dumps, Gold v2, or the paper.
Run: CUDA_VISIBLE_DEVICES=2 python -u scripts/run_llama3_8b_sopv4_p2_2601.py
"""
...
MODEL_PATH = ROOT / "LLaMA-Factory/Meta-Llama-3-8B-Instruct"
ALT_MODEL = MODEL_PATH
...
def freeze_now(p2_rows: list[dict]) -> dict:
    ...
    freeze = {
        ...
        "torch_dtype": "bfloat16",
        "quantization": "none",
        "lora": "none (Instruct zero-shot; not sft_skillspan_ner)",
...
def score_all(jieba_rows: list[dict]) -> dict:
    ...
    write_csv(REPORT_DIR / f"{RUN}_on_gold_v2_diagnostic.csv", [r1])
...
def main() -> int:
    ...
    md = REPORT_DIR / "llama3_8b_instruct_sopv4_error_summary.md"
    lines = ["# Llama-3-8B-Instruct SOP-v4 P2 error summary", "", f"Run `{RUN}`.", "", "| Kind | Count |", "|---|---:|"]
