# 980 句标注指南（手册 B / SOP v4.2.9）

**用途：** 给标注员。协议与 `notes/handbooks/handbook_B_sop_v4.md` 相同。  
**手册版本：** `B.sop_v4.2.9`。四类定义以 **ESCO 技能柱原文**为权威。切段只有一条：语义不变则尽量短。工具名按谓语分流。××能力两步；网络/安全/ICT 三语境；保证/确保不进跨度；经验默认剥。  
**出处对照：** `handbook_B_citations.md`。键：[ESCO14] [EQF] [ESCO-L] [ESCO-T] [ESCO-Q] [Z22] [Say18] [AP08] [Kr95] [TKS02] [FM09] [Yu20] [UD20] [D18]。`[本协议]` = 本资源操作化，不是被一篇论文强制的金标准。  
...
| `k` | K | 学历/资格名、专业、领域知识、非语言职业认证；课表/原理里的语言名 | 岗位运用中的 Python / Spring（那是 **S**）；英语六级（那是 **L**）；把学历整段留空 | [EQF] [ESCO-Q] [Z22] |
| `s` | S | 可执行职业技能；**岗位运用中的**工具/语言/框架 | 整段岗位职责；课表/原理里的 Python（那是 **K**）；沟通管理因「负责」改 S | [EQF] [ESCO14] [本协议] |
| `t` | T | 跨行业软技能、特质；含沟通管理 / 客户汇报 | 形象外貌、身体健康；职业动作「对接」 | [ESCO-T] [Z22] [Say18] [本协议] |
...
`分析推广效果`、`制定推广策略`、`优化曝光与转化率` → S。「保证/确保」不进跨度；纯结果整段不标；有技术对象只标对象。[本协议]  
「××能力」：绑工具/方法 → S；换行业还能用 → T。禁止「能做事→S」。`科研能力` 默认 T。[ESCO-T] [ONET] [本协议]  
`会聊天` / `会说话` / `会来事` / `会做人` → 整段 T，**含「会」**。`会使用Excel` 仍剥「会」。

## 跨度规则（必须）

1. 跨度是句中**连续原文**。不要改写、不要补字。[Z22] [D18]  
2. **禁止半词**：不要 `支持服`、`操作系统的问`、`培训其`。[本协议]  
3. **唯一切段：语义不变则尽量短**（优先 2–8 字）。`Python`、`计算机专业`、`沟通能力`。不要一整段职责收成一条 S。**废止**「能不拆就不拆 / 一条外层长 S」。**[本协议]** 切段见 [AP08]。  
4. 顿号/逗号/「和、与、或」连接的**独立**能力 → **各自一条**。[Z22] 只有拆开会半词或类型会漂，才留一条。[UD20] [本协议]  
5. 「熟悉 / 掌握 / 了解 / 精通 / 具备 / 具有」**只标后面的对象**，不标这些动词。[Z22] [本协议]  
6. 编程语言、框架、办公软件、具体工具**不是固定一类**：运用/岗位工具 → 光杆名 **S**；课程/原理/基础/语法 → **整段知识短语 K**（`Python语言原理`，不要只圈 `Python`）；看不出 → **S** 并写不确定。[EQF] [本协议]  
7. 语种名、语言水平、语言考试（大学英语六级 / CET-6 / 日语N2）→ **L**。ISO / OCJP 等技术或职业认证 → **K**。[ESCO-L] [本协议]  
8. 平坦、不重叠。一条跨度一个类型。不要嵌套。[TKS02] [本协议] 嵌套是另一项任务 [FM09] [Yu20]。L–K–S–T **不是**优先级；**禁止** `L＞S＞K＞T` [ESCO14] [Z22] [AP08]。成对检验后仍拿不准 → 标 **S** 并写裁决日志 [Z22]。潜在同界/嵌套/交叉：能拆就拆；否则只留完整能力，备选写入裁决日志，不要猜 [AP08] [Kr95] [Z22]。  
9. 不标：形象外貌、身体健康、年限数字本身（只留能力）、非司机岗驾照、留学优先套话。[Z22] [本协议] 学历/资格**名**要标 **K**（`本科及以上学历`），不要写成学历壳不标 [Z22] [ESCO-Q]。  
10. `沟通管理` / `客户汇报` → **T**，即使写在「负责…工作」里 [ESCO-T] [Z22]。职业动作（`对接`）仍 S。  
11. `网络` / `安全` / `ICT`：认证/专业 **K**；规划运维 **S**；行业修饰不标。[Nav09] [本协议]  
12. `工作经验` 不标。行内话剥「经验」：`大项目售前经验` → `大项目售前` S；`数据分析经验` → `数据分析` S。[本协议]
...
| 负责与客户的沟通管理…客户汇报 | `沟通管理` **T**；`客户汇报` **T** | 因职责改 S | [ESCO-T] [Z22] |
