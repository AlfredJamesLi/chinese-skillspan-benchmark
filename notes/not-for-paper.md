...
- Merging Handbook A (Gold-length P1) and Handbook B (short SOP P2) into one guideline, overwriting `gold_canonical_v2.jsonl`, or calling the 980 SimHuman overlay a full human SOP gold
- Claiming Handbook A is a reprint of the original `prompt_template_rag.py` silver API (that API used minimal sufficient span and put language certificates in L)
