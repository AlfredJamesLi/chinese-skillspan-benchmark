...
新一轮人工编码使用[中文手册B.sop_v4.2.14](handbook_B_sop_v4.md)。[培训目录](training/README.md)提供培训流程、32个历史案例及登记空表，整理日期为2026年9月26日；实际培训与试测完成情况须以填写后的记录为准。

下文保留A/B/C手册和旧实验方案的历史说明。其中旧主评测数字、历史队列及旧版状态不代表当前论文和本轮试测进度。
...
**v4.2.6（2026-09-05）：** **唯一切段：语义不变则尽量短。** 废止 guideline「能不拆就不拆 / 一个动词一条长 S」。嵌套 Long_S 明确留给下一篇文章。不改论文数字。

**v4.2.7（2026-09-05）：** 冻结第 3–4 点。同一工具名按谓语分流：运用/岗位工具 → 光杆名 **S**；了解/课程/原理 → 整段 **K**；看不出 → **S**。不要写成 ESCO/SkillSpan 词表规定。不改论文数字。

**v4.2.8（2026-09-05）：** 收口剩余五条操作检验（××能力、一词三用、保证/确保、经验、Gold 准入）。不改项目 22 全量、不改论文数字。

**v4.2.9（2026-09-05）：** `售前` 为行内话，`大项目售前经验` → `大项目售前` S，默认剥「经验」。
