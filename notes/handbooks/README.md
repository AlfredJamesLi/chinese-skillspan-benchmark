...
**v4.1（2026-08-28）：** 手册 B 仍是权威 SOP。新增重叠裁决附录 `handbook_B_overlap_adjudication.md` 与规则日志 `LSKT_V4_RULE_CHANGELOG.md`。证书（CET-6 / 英语六级）**仍为 K**（Codex 草案把证书改 L，未采纳）。新提示词：`prompts/LSKT_V4_ANNOTATION_PROMPT.txt`。人标队列：`reports/human980_doccano/`（草稿）。标准化协议双盲 IAA 与裁决后 Gold **尚未存在**。
