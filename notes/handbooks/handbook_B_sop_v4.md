...
**手册版本：** `B.sop_v4.2.11`（2026-09-07；Silver-plus 提示词复核后修订）。以 ESCO 概念为参照，中文招聘操作化以本协议明文为准；不得把学历映射、行业经验代理、工具语境分流说成 ESCO 原文。**唯一切段：语义不变则尽量短，完整性先于长度**。工具名按语境分流。主层仅做连续原文、平坦不重叠的抽取。经验按下文分情形处理。重叠裁决与 Gold 准入见 `handbook_B_overlap_adjudication.md`；出处对照 `handbook_B_citations.md`；规则日志 `LSKT_V4_RULE_CHANGELOG.md`。本次新增条款见 R08–R13；历史数据不追溯改版。  
