# Codex 任务：wave2 — gpt-6-astra 标剩余 Silver

你在仓库 `/home/guojingli3/SCESC-LLM-skill-extraction` 的 **Chinese-SkillSpan** 窗口。
**只做这一波：wave2。** 不要做 wave1/wave2/wave3 中的其它波，不要重标 `ids_held_2500.txt` 的 2500 句。
不要改 `access_paper/`、Gold150、V4 hybrid、`data/silver_plus_v6a_nocross/`、`run2500_public_api_v1/`。不要写论文 F1。

## 本波

目录：`Chinese_skill_benchmark_Paper/data/silver_plus_10k_prep_20260913/codex_remaining_5436/waves/wave2/`

- 句子 **1810**；JSON 批 **181**（`batch_0182.json` … `batch_0362.json`）
...
全部 181 批写完后，把 `PROGRESS.json` 的 `wave2.status` 改成 `done`（若没有该文件，在本波目录写 `STATUS.json`：`{"wave":"wave2","n_out": <已写批次数>}`）。
