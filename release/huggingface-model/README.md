...
This repository’s Hub field remains `license: other`. Academic research use of the advertisement text is confirmed (`DATA_AVAILABILITY.md`); that is **not** a CC-BY grant on the wording or on these weights. The GitHub paper repository proposes Apache-2.0 for **software** only (`LICENSE`). Compatibility with Apache-2.0 of the backbone is required before any more permissive SPDX id is chosen.
