...
**Human page-1 200（2026-09-03）：** 980 分歧队列前 200 句人标已入库 `data/human_gold_page1_200.jsonl`。叠回 hybrid 的诊断表：JobBERT 3M 2601 exact **0.4331→0.3884**（`data/test_lskt_v4_hybrid_human200_cws.jsonl`）。**不覆盖** 原 hybrid 与 Gold v2。摘要仍用 **0.4331**。Overleaf 只贴一份：`overleaf_cursor_bundle/CODEX_PROMPT_HUMAN200_OVERLEAF.md`。980 全部人标后必须重打分；不必仅为测试金标重训 CRF。
